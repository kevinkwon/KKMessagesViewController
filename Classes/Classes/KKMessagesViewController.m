...
#import "UIColor+KKMessagesView.h"
#import "UIButton+KKMessagesView.h"

@interface KKMessagesViewController () <KKDismissiveTextViewDelegate>
...
@implementation KKMessagesViewController
...
- (void)setup
{
    ...
    CGRect tableFrame = CGRectMake(0.0f, 0.0f, size.width, size.height - [KKMessageInputView defaultHeight]);
    ...
    [self setBackgroundColor:[UIColor kk_messagesBackgroundColor_iOS6]];
    
    CGRect inputFrame = CGRectMake(0.0f,
                                   size.height - [KKMessageInputView defaultHeight],
                                   size.width,
                                   [KKMessageInputView defaultHeight]);
    
    KKMessageInputView *inputView = [[KKMessageInputView alloc] initWithFrame:inputFrame
    ...
    if([self.delegate respondsToSelector:@selector(sendButtonForInputView)]) {
        ...
        sendButton = [UIButton kk_defaultSendButton_iOS6];
    }
...
- (void)sendPressed:(UIButton *)sender
{
    [self.delegate didSendText:[self.messageInputView.textView.text kk_stringByTrimingWhitespace]];
}
...
{
    KKBubbleMessageType type = [self.delegate messageTypeForRowAtIndexPath:indexPath];
    ...
    KKBubbleMessageCell *cell = (KKBubbleMessageCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    
    if(!cell) {
        cell = [[KKBubbleMessageCell alloc] initWithBubbleType:type
...
{
    return [(KKBubbleMessageCell *)[self tableView:tableView cellForRowAtIndexPath:indexPath] height];
}
...
- (BOOL)shouldHaveTimestampForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch ([self.delegate timestampPolicy]) {
        case KKMessagesViewTimestampPolicyAll:
            return YES;
            
        case KKMessagesViewTimestampPolicyAlternating:
            return indexPath.row % 2 == 0;
            
        case KKMessagesViewTimestampPolicyEveryThree:
            return indexPath.row % 3 == 0;
            
        case KKMessagesViewTimestampPolicyEveryFive:
            return indexPath.row % 5 == 0;
            
        case KKMessagesViewTimestampPolicyCustom:
...
- (BOOL)shouldHaveAvatarForRowAtIndexPath:(NSIndexPath *)indexPath
{
	switch ([self.delegate avatarPolicy]) {
        case KKMessagesViewAvatarPolicyAll:
            return YES;
            
        case KKMessagesViewAvatarPolicyIncomingOnly:
            return [self.delegate messageTypeForRowAtIndexPath:indexPath] == KKBubbleMessageTypeIncoming;
			
		case KKMessagesViewAvatarPolicyOutgoingOnly:
			return [self.delegate messageTypeForRowAtIndexPath:indexPath] == KKBubbleMessageTypeOutgoing;
            
        case KKMessagesViewAvatarPolicyNone:
        default:
            return NO;
    }
}
...
- (BOOL)shouldHaveSubtitleForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch ([self.delegate subtitlePolicy]) {
        case KKMessagesViewSubtitlePolicyAll:
            return YES;
        
        case KKMessagesViewSubtitlePolicyIncomingOnly:
            return [self.delegate messageTypeForRowAtIndexPath:indexPath] == KKBubbleMessageTypeIncoming;
            
        case KKMessagesViewSubtitlePolicyOutgoingOnly:
            return [self.delegate messageTypeForRowAtIndexPath:indexPath] == KKBubbleMessageTypeOutgoing;
            
        case KKMessagesViewSubtitlePolicyNone:
        default:
            return NO;
    }
}
...
{
    CGFloat maxHeight = [KKMessageInputView maxHeight];
    ...
    self.messageInputView.sendButton.enabled = ([textView.text kk_stringByTrimingWhitespace].length > 0);
}
...
- (void)keyboardWillShowHide:(NSNotification *)notification
{
    ...
    [UIView animateWithDuration:duration
                     ...
                     animations:^{
                         ...
                         CGFloat messageViewFrameBottom = self.view.frame.size.height - [KKMessageInputView defaultHeight];
