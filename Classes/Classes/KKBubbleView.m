...
- (void)setType:(KKBubbleMessageType)type
{
    _type = type;
    [self setNeedsLayout];
}
...
- (CGRect)bubbleFrame
{
    ...
    return CGRectMake((self.type == KKBubbleMessageTypeOutgoing ? self.frame.size.width - bubbleSize.width : 0.0f),
                      kMarginTop,
                      bubbleSize.width,
...
- (void)layoutSubviews
{
    ...
    if(self.type == KKBubbleMessageTypeIncoming) {
...
- (CGSize)textSizeForText:(NSString *)txt
{
    ...
    CGFloat maxHeight = MAX([KKMessageTextView numberOfLinesForMessage:txt],
                         [txt kk_numberOfLines]) * [KKMessageInputView textViewLineHeight];
    maxHeight += kKKAvatarImageSize;
