...
@implementation KKMessageInputView
...
//    self.image = [[UIImage imageNamed:@"input-bar-background"] resizableImageWithCapInsets:UIEdgeInsetsMake(19.0f, 3.0f, 19.0f, 3.0f)];
    ...
    CGFloat height = [KKMessageInputView textViewLineHeight];
    
    KKMessageTextView *textView = [[KKMessageTextView  alloc] initWithFrame:CGRectMake(6.0f, 3.0f, width, height)];
...
- (void)adjustTextViewHeightBy:(CGFloat)changeInHeight
{
    ...
    int numLines = MAX([self.textView numberOfLinesOfText],
                       [self.textView.text kk_numberOfLines]);
...
+ (CGFloat)maxHeight
{
    return ([KKMessageInputView maxLines] + 1.0f) * [KKMessageInputView textViewLineHeight];
}
